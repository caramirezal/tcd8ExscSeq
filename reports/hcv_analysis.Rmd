...
geom.bar.me3 <- me3 %>%
  ggplot(aes(x=`Significant in KO`, 
        ...
        theme_bw() +
        theme(legend.position = 'none') +
             labs(x='Up/Down regulation in UTX mutant',
             y='Log2FC in H3K27me3 in UTX mutant',
             fill='') +
...
#sign.list$utx.sign.down %>% unique() %>% length()
...
```{r}
miller.me3.dep.plot <- dittoPlot(subset(miller, 
                                     cell_type %in% c('Progenitor Ex', 'Terminally Ex')), 
          "UTX_dep", 
          group.by = "cell_type",
    plots = c("jitter", "vlnplot", "boxplot"),
...
miller.me3.ind.plot <- dittoPlot(subset(miller, 
                                     cell_type %in% c('Progenitor Ex', 'Terminally Ex')), 
          "UTX_ind", 
          group.by = "cell_type",
    plots = c("jitter", "vlnplot", "boxplot"),
...
DotPlot(subset(miller, cell_type %in% c('Progenitor Ex',
                                         'Terminally Ex')), 
...
```{r}
DotPlot(subset(miller, cell_type %in% c('Terminally Ex', 'Progenitor Ex')),
        features = me3.sign$me3.dep,
        group.by = 'cell_type',
        dot.scale = 15) +
      theme(axis.text.x = element_blank())
